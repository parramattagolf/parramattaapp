...
### 6. 1촌 신청 및 거절 규칙 (Friend Request Logic)
- **자격 제한**:
    - **발신 제한**: **RED** 등급은 1촌 신청 불가 (프로필 미완성, 수정 페이지 이동 팝업).
    - **수신 제한**: **RED** 등급 회원에게는 1촌 신청 불가 (버튼 대신 경고 문구 "⚠️ 신뢰하기 어려운 회원").
    - **YELLOW** 이상부터 신청 및 수신 가능.
- **버튼 상태 (Button Types)**:
    - **신청 전**: `[1촌신청]` (Blue)
    - **신청 중 (발신)**: `[1촌신청중]` (Gray, Disabled)
    - **신청 수신**: 프로필에서 `[수락] / [거절]` 버튼 노출.
    - **1촌 관계**: `[1촌관계]` (Blue Badge)
    - **거절 됨**:
        - 1회 거절 시: `[1촌재신청]` (Red) - 재도전 가능.
        - 2회 거절 시: `[1촌거절]` (Black) - 영구 신청 불가 (Disabled).
- **촌수 표시**:
    - 신청 버튼 좌측에 `2촌관계`, `3촌관계` 텍스트 표시 (3촌까지 추적).
